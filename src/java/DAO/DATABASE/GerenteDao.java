...
public class GerenteDao {
    ...
    public void insertGerente(Gerente user) throws SQLException {
      ...
      public boolean updateVendedor(Gerente user) throws SQLException {
        String sql = "UPDATE gerente SET nome=?, sexo=?, apelido=?, naturalidade=?,"
                + " identidade=?, estadocivil=?, especialidade=?, eMail=?, senha=?, "
                + "idade=?, telefone=?, dataDeNascimento=?, dataDoContrato=?, salario=?,"
                + " estado=?, acessoNivel1=?, acessoNivel2=?, acessoNivel3=? WHERE id=?";

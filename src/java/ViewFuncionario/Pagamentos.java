...
public class Pagamentos extends javax.swing.JPanel {
    ...
    private Venda venda;
    private double precoFinal,precoMes;
    private String tipPagamento;
    private vendedorFrame frame;
        DecimalFormat decimalFormat = new DecimalFormat("0.00");

    public Pagamentos(Funcionario funcionario, Carro carro, Cliente cliente, vendedorFrame frame) {
        this.frame = frame;
        ...
        txValorDeEntrada.setVisible(false);
    ...
    private void initComponents() {
        ...
        BasePagamento.add(precoPorMes, new org.netbeans.lib.awtextra.AbsoluteConstraints(240, 290, 290, 40));
        ...
        BasePagamento.add(cambioTipoDeCombustivel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(260, 190, 290, 40));
        ...
        BasePagamento.add(txValorDeEntrada, new org.netbeans.lib.awtextra.AbsoluteConstraints(610, 200, 270, 40));

        valor.setFont(new java.awt.Font("Roboto Medium", 0, 14)); // NOI18N
        valor.setText("Valor de entrada");
        BasePagamento.add(valor, new org.netbeans.lib.awtextra.AbsoluteConstraints(610, 130, 120, 30));

        entrada.setFont(new java.awt.Font("Roboto Medium", 0, 11)); // NOI18N
        BasePagamento.add(entrada, new org.netbeans.lib.awtextra.AbsoluteConstraints(740, 130, 210, 30));
    ...
    private void PrestacoesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_PrestacoesActionPerformed
        ...
        txValorDeEntrada.setVisible(true);
    ...
    private void ComboPrestacoesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ComboPrestacoesActionPerformed
        ...
        if (ComboPrestacoes.getSelectedIndex() == 0) {
            precoApagar.setText("Valor " +decimalFormat.format(venda.precoFinalComPrestacoes(1))  + " MZN");
            precoFinal = venda.precoFinalComPrestacoes(1);
            
            
            lbPrestacoes.setText(" Vai pagar " + decimalFormat.format(venda.precoFinalComPrestacoes(1)/6)+" Por Mez");
            precoMes =(venda.precoFinalComPrestacoes(1)/6);
               entrada.setText("Deve ser maior ou igual : "+decimalFormat.format(precoFinal*0.20)  );
            ...
            precoFinal = venda.precoFinalComPrestacoes(2);
              lbPrestacoes.setText(" Vai pagar " + decimalFormat.format(venda.precoFinalComPrestacoes(1)/12)+" Por Mez");
               precoMes =(venda.precoFinalComPrestacoes(1)/12);
                 entrada.setText("Deve ser maior ou igual : "+ decimalFormat.format(precoFinal*0.20 ) );
        } else if (ComboPrestacoes.getSelectedIndex() == 2) { 
            precoApagar.setText("Valor " + decimalFormat.format( venda.precoFinalComPrestacoes(3)) + " MZN");
            precoFinal = venda.precoFinalComPrestacoes(3);
             precoMes =(venda.precoFinalComPrestacoes(1)/12);
              lbPrestacoes.setText(" Vai pagar " + (venda.precoFinalComPrestacoes(1)/8)+" Por Mez");
                entrada.setText("Deve ser maior ou igual : "+ decimalFormat.format(precoFinal*0.20 ) );
        }
    ...
    private void AvancarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_AvancarActionPerformed
        ...
        if (Avista.isSelected()) {
            ...
            venda.setCarro(carro);
            FinacasDao daos = new FinacasDao();
            ArrayList<Financas> openFile1 = daos.openFile();
            openFile1.get(0).setSaldo((openFile1.get(0).getSaldo() + precoFinal));
            ...
            if(!( Double.parseDouble((txValorDeEntrada.getText()))<0.20*precoFinal)){
            venda.setAnoDaGarantia(c.get(Calendar.YEAR + 1));
            venda.setAnoDaVenda(c.get(Calendar.YEAR));
            venda.setCarro(carro);
            venda.setCliente(cliente);
            ...
            presta.setValarMinimoMes(precoMes);
            presta.setStatus(false);
               
            double valorEntrada = Double.parseDouble(txValorDeEntrada.getText());
            FinacasDao fd = new FinacasDao();
            ArrayList<Financas> openFile1 = fd.openFile();
            openFile1.get(0).setSaldo(openFile1.get(0).getSaldo() + valorEntrada);
            ...
            aO.insert(presta);
                ...
                Notifications.getInstance().show(Notifications.Type.ERROR, "O Valoe de Entdada E menor Que 20% Do valor Total");
            }
        }
